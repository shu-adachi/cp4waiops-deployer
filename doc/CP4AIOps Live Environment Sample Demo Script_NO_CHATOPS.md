...
**Slide 10**</br>
オペレーションやSREが運用手順書を作成した場合、WAIOPSは自動的に運用手順書を起動し、問題を軽減することができます。
...
**ストーリー**には、関連するログの異常、トポロジー、類似のインシデント、過去のトラブルチケットに基づく推奨アクション、関連イベント、運用手順書などが含まれています。
...
1. Watson AIOpsが最も可能性の高い原因として推定したアラートのリスト
2. トポロジーに関連する問題を局所化
3. インシデントを自動的に軽減するための提案された運用手順書
4. 現在発生しているインシデントと同様の、過去の解決チケット
...
>**🚀 <u>アクション</u>** <br/>
タイプ欄に **ANOMALY:** とあるアラート行をクリックします。次に、**メトリック異常の詳細** アコーディオンを開きます。

**📣 <u>ナレーション</u>**

* Cloud Pak for Watson AIOpsは、複数のソースからメトリクスを収集し、**メトリック異常** を検出することも可能です。環境からの数百または数千のメトリクスでトレーニングされ、動的なベースラインを構築します（緑で表示）。グラフィックが突然赤くなるのは、データベースが通常よりも大量のメモリを消費しているときに検出された異常に関連しています。
